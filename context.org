...
- [ ] Once Festival event is created, registering for Crossroads course also registers for One Finger Shooting Zen (maybe show courses registered on confirmation page with special note about courses registered that the user didn't explicitly pick?). This should ideally not be hard-coded for Festival but be based on inspecting discounts to see if adding any courses will result in no price difference (note that inspection will already happen to auto-apply relevant discounts for cheapest price)
