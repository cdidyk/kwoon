...
- [ ] Event model exists with the following fields:
  - title
  - description
  - location
  - start date
...
- [ ] Event Discount model exists with the following fields:
  - event Id
  - description
  - course Ids (array or comma-delimited)
  - price (in cents)
- [ ] Courses can now optionally belong to an Event
- [ ] Course start/end dates must be within Event's start/end dates
- [ ] Either Courses should have an optional base price or there should be an event_courses table that has a price (leaning towards former)
- [ ] Events are expected to have a corresponding view in app/views/events/<id>-<title>
- [ ] Event page has a list of event courses (chronological by-end-date) with checkboxes and prices
- [ ] Checking and unchecking courses changes the total displayed, taking any discounts into account
- [ ] Once Festival event is created, selecting Crossroads course also selects One Finger Shooting Zen
*** TODO Users can register for Events
**** [0/14] Acceptance Criteria
- [ ] Event Registration model exists with the following fields:
  - event id
  - user id
  - amount paid
  - stripe id (for payment)
- [ ] Registering for an event creates an Event Registration
- [ ] Registering for an event creates Course Registrations for each course the user registered for
- [ ] The current Registration model (and table) should become CourseRegistration
- [ ] The event page has a user and CC info section with a "Register" button
- [ ] User model has a from/location column (optional) for capturing city/state (if USA) or city/country (if not USA)
- [ ] Registrations for an email not already in the system create a new user and new stripe customer
- [ ] Registrations for an email already in the system use the existing user/stripe customer
- [ ] The CC used should become the stripe customer's default source
- [ ] Event registrations are one-time payments (pay-in-full) only
- [ ] Event registration payments should attach the event name + registered course titles to the stripe payment for easy lookup later
- [ ] At least one course must be chosen
- [ ] Validation works like course registration
*** TODO Emails are sent using a transactional email service
** [0%] Release 3
