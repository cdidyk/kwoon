...
**** TODO Applicants pick their interest
...
- [ ] On application, there is a section for interests with the following (checkbox) options:
  - Shaolin Cosmos Chi Kung
  - Shaolin Kung Fu
  - Wahnam Tai Chi Chuan(?)
  - Courses with Grandmaster Wong Kiew Kit
- [ ] At least one interest must be selected
- [ ] Existing users with applications should be set as interested in Shaolin Kung Fu
- [ ] Existing users with no applications (i.e. Festival attendees) should be set as interested in Courses with Grandmaster Wong (and other relevant interests based on what I know about them)
